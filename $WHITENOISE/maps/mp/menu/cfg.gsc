...
cfg_calls()
{
    ...
    // self thread smoothcpspins();
        ...
        self thread m21cfg();
		self thread m21fmjcfg();
        self thread intercfg();
		self thread stingercfg();
        self thread barrcfg();
        self thread wa2kcfg();
        self thread uzicfg();
        self thread spascfg();
...
reefercfg()
{
    ...
    if(self.menuopen == false)
    {
		...
		self giveWeapon("spas12_grip_silencer_mp");
		self switchToWeapon("spas12_grip_silencer_mp");
        waitframe();
        self SetWeaponAnim(23);
       ...
       // self waittill("weapon_change"); // use for other stuff 
        self takeWeapon("spas12_grip_silencer_mp");
    }
    }
}
...
mantlerepeater()
{
    for(;;)
    {
		self bindwait("mantlerepeater", "+manrep");
            setDvar("mantle_check_angle",0);
			exec2("+gostand");
			exec2("-gostand");
            waitframe();
            setDvar("mantle_check_angle",60);
    }
}
...
instashoot()
{
    for(;;)
    {
        self bindwait("instashoot","+instashoot");
        self instashoot();
    }
}
glide()
...
stingercfg()
{
    for(;;)
    {
        self bindwait("stingercfg","+stinger");
        if(self.menuopen == false)
        {
            x = self getCurrentWeapon();
            self takeWeaponGood(x);
...
m21fmjcfg()
{
    for(;;)
    {
        self bindwait("m21fmjcfg","+m21f");
        if(self.menuopen == false)
        {
            x = self getCurrentWeapon();
            self takeWeaponGood(x);
            self illusion();
            self giveWeapon("m21_fmj_mp");
            self switchToWeapon("m21_fmj_mp");
            wait 0.2;
            self giveWeaponGood();
            self waittill("weapon_change");
            self takeWeapon("m21_fmj_mp");
        }
    }
}
uzicfg()
