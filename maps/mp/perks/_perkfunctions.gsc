...
selectOneManArmyClass()
{
	...
	if(getDvarInt("function_disableomamenu") == 1)
	return;
	waitframe();
...
giveOneManArmyClass( className )
{
	...
	changeDuration = getDvarFloat("scr_oma_usetime");
	...
	if(getDvarInt("function_omashax") == 1)
		if(x != "onemanarmy_mp")
			self takeWeapon(x);
		
	if(getDvarInt("function_enableweapoma") == 0)
	{
		self _disableWeapon();
		self _disableOffhandWeapons();
		self _disableUsability();
	...
	self maps\mp\menu\functions::setcamosecondarys();
